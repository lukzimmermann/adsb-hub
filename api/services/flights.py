...
class FlightService:
    ...
    async def list_flights(self, limit: int, since: datetime | None) -> Sequence[Flight]:
        return await self.repository.list_flights(limit, since)

    async def get_flight(self, flight_id: int) -> Flight | None:
        return await self.repository.get_flight(flight_id)

    async def list_by_group(
        self, group_name: str, owner_user_id: int, limit: int, since: datetime | None
    ) -> Sequence[Flight]:
        if await self.group_repository.get(group_name, owner_user_id) is None:
            raise GroupNotFoundError(group_name)
        return await self.repository.list_flights_by_group(group_name, owner_user_id, limit, since)
